...
## Take a peek at the data {.tabset}

### Getting the descriptives for **all** the variables  
```{r}
describeBy(data, group = data$timepoint_num)
```

### Density plots for a few of the ones we might be interested in.
```{r}
...
LIWCvar <- c("WC","Analytic","Cognition","cogproc","cause","tentat","Drives","emo_pos", "emo_neg","emo_anx",
             'focuspast','focuspresent','focusfuture','i',"achieve")
...
ggplot(data = data, 
  ...
  geom_density(aes_string(x = LIWCvar[i]),
               adjust = 1.5, 
               alpha = 0.4) +
  labs(title = paste("Density Plot for", LIWCvar[i]))
}
...
ggarrange(d1,d2,d3,d4,d5,d6,d7,d8,d9,d10,d11,d12,d13,d14, common.legend = T, legend = 'right')
```
