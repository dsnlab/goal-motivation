...
mod_h1 <- brm(progress ~ timepoint_num + autonomous_motivation_between + autonomous_motivation_between * timepoint_num + autonomous_motivation_within +
              ...
              prior = h1_prior, data = data, sample_prior = TRUE,  chains = 4,
  iter = 1e4,
  warmup = 1000)
```
...
mod_e1 <- brm(progress ~ timepoint_num + controlled_motivation_between + controlled_motivation_between * timepoint_num + controlled_motivation_within +
              ...
              prior  =  e1_prior, data = data, sample_prior = TRUE,chains = 4,
  iter = 1e4,
  warmup = 1000)
...
am_within <- 
  predicted %>%
  ggplot(aes(group, predicted)) +
  geom_line(data = filter(within, split == "high"), aes(group = interaction(SID)), alpha = .25, color = palette_condition[3]) +
  geom_line(data = filter(within, split == "low"), aes(group = interaction(SID)), alpha = .25, color = palette_condition[1]) +
  geom_pointrange(aes(color = x, group = x, ymin = conf.low, ymax = conf.high), position = position_dodge(0.05), size = 1) +
  geom_line(aes(color = x, group = x), position = position_dodge(0.05), size = 1.5) +
  scale_color_manual(name = "Motivation", values = palette_condition) +
  scale_x_discrete(expand = c(.1, .1)) +
...
am_graphs <- ggpubr::ggarrange(am_between,am_within ,ncol=1, nrow=2, common.legend = T, legend = "top")
am_graphs
...
cm_graphs <- ggpubr::ggarrange(cm_between,cm_within ,ncol=1, nrow=2, common.legend = T, legend = "top")
cm_graphs
```

## All together
```{r fig.height=20, fig.width=28, message=FALSE}
all_graphs <- ggpubr::ggarrange(am_between,am_within,cm_between,cm_within,col = 2, nrow=2,common.legend = T, legend = "top")
all_graphs
```
